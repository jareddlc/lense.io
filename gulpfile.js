...
gulp.task('d3-js', function() {
  return gulp.src(D3.js.files).pipe(gulp.dest(D3.js.dest));
});

gulp.task('trianglify', ['trianglify-js']);
gulp.task('trianglify-js', function() {
  return gulp.src(TRIANGLIFY.js.files).pipe(gulp.dest(TRIANGLIFY.js.dest));
});

// Default Taskp
gulp.task('default', ['angular', 'bootstrap', 'jquery', 'd3', 'trianglify']);